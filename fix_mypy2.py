...
fixes = [
    ("core/repositories/base.py", [
        ...
        ('self.model.deleted_at.is_(None) if hasattr(self.model, "deleted_at") else True,', 'getattr(self.model, "deleted_at").is_(None) if hasattr(self.model, "deleted_at") else True,  # type: ignore'),
        ("entity.deleted_at = datetime.now(timezone.utc)", "setattr(entity, 'deleted_at', datetime.now(timezone.utc))  # type: ignore")
    ])
]

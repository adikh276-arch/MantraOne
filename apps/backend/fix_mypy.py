...
fixes = [
    (
        ...
        [
            (
                'def __gt__(self, other: "SignalSeverity") -> bool:',
                "def __gt__(self, other: object) -> bool:\n        if not isinstance(other, SignalSeverity): return NotImplemented",
            ),
            (
                'def __ge__(self, other: "SignalSeverity") -> bool:',
                "def __ge__(self, other: object) -> bool:\n        if not isinstance(other, SignalSeverity): return NotImplemented",
            ),
        ],
    ),
    ("config/settings.py", [("@computed_field\n    @property", "@property\n    @computed_field")]),
    (
        "core/repositories/base.py",
        [
            (
                "return await self._db.get(self._model, id)",
                "return await self._db.get(self._model, id)  # type: ignore",
            ),
            ...
            (
                "await self._db.execute(select(self._model).where(*conditions).offset(skip).limit(limit))",
                "await self._db.execute(select(self._model).where(*conditions).offset(skip).limit(limit))  # type: ignore",
            ),
            (
    ...
    ("alembic/env.py", [("from alembic import context", "from alembic import context  # type: ignore")]),
    ("core/events/subscriber.py", [("results = await self._redis.xread", "results: list = await self._redis.xread")]),
    (
        "core/providers/retrieval_service.py",
        [
            (
                "await self._memory.get_health_context(member_id, family_id, domains, lookback_days)",
                "await self._memory.get_health_context(member_id=member_id, family_id=family_id, include_domains=domains, lookback_days=lookback_days)",
            )
        ],
    ),
    (
    ...
    ("core/services/document_service.py", [("provider.compute_checksum", "provider.compute_checksum  # type: ignore")]),
]

...
async def main():
    ...
    async with httpx.AsyncClient(base_url=BASE_URL, headers=HEADERS) as client:
        ...
        r = await client.post(
            "/v1/families/", params={"name": data["name"], "primary_member_name": data["primary_member"]["name"]}
        )
        ...
        r = await client.post(
            "/v1/conversations/", json={"family_id": family_id, "member_id": family_id, "title": "Golden Baseline"}
        )
        ...
        async with client.stream(
            ...
            json={
                "family_id": family_id,
                "member_id": family_id,
                "text": "Syncing baseline medical memory. Please confirm.",
            },
        ) as response:

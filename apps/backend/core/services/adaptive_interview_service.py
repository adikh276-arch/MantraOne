    ...
    QUESTION_LIBRARY = {
        ...
        "missing_lab": {
            ...
            "options": [
                ...
                "No, I need to schedule a test",
                "My doctor said I don't need it yet",
            ],
        },
        "symptom_progression": {
    ...
    def generate_question(self, domain: str, context_vars: Dict[str, str]) -> Dict[str, Any]:
        ...
        if domain in self.QUESTION_LIBRARY:
            ...
            try:
                ...
                return {"question_text": text, "type": "MCQ", "options": q_template["options"]}
            except KeyError:
                # Missing context var, fallback
                pass

        # LLM Fallback (mocked for this implementation, would call LLMProvider)
        return {"question_text": f"Could you provide an update regarding {domain}?", "type": "OPEN", "options": []}

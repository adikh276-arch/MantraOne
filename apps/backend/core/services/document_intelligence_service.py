...
class DocumentExtractionResult(BaseModel):
    ...
    document_date: Optional[date] = Field(
        None, description="The primary date the document was issued or relates to, in YYYY-MM-DD format"
    )
    ...
    {"name": "string", "type": "string", "confidence": 0.9}
        ...
        try:
            raw_dict = await self._llm.complete_structured(
                self.SYSTEM_PROMPT, prompt, response_schema=DocumentExtractionResult, max_tokens=1500
            )
            ...
            return DocumentExtractionResult(
                document_type="unknown", confidence=0.0, summary="Failed to extract metadata."
            )

    async def determine_semantic_duplicate(
        self, extracted: DocumentExtractionResult, member_id: UUID
        ...
        res = await self._db.execute(
            select(Document).where(Document.member_id == member_id, Document.document_date == extracted.document_date)
        )

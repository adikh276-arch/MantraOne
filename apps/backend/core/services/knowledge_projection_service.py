...
from infrastructure.database.models import FamilyMember, HealthInsight, FollowUp, DomainConfidence, PreventiveObservation
...
class KnowledgeProjectionService:
    ...
    async def build_family_graph(self, family_id: UUID) -> Dict[str, Any]:
        ...
        for m in members:
            ...
            for c in confs:
                member_node["confidences"].append({"domain": c.domain, "confidence": c.confidence, "freshness": c.freshness})
                
            # Insights
            res_ins = await self._db.execute(select(HealthInsight).where(HealthInsight.member_id == m.id, HealthInsight.status == "active"))
            insights = res_ins.scalars().all()
            for i in insights:
                member_node["insights"].append({"type": i.insight_type, "desc": i.description})
                
            # Follow-ups
            res_fu = await self._db.execute(select(FollowUp).where(FollowUp.member_id == m.id, FollowUp.status.in_(["scheduled", "pending"])))
            fus = res_fu.scalars().all()
            for fu in fus:
                member_node["follow_ups"].append({"desc": fu.description, "due": fu.due_date.isoformat() if fu.due_date else None})
                
            # Preventive Needs
            res_prev = await self._db.execute(select(PreventiveObservation).where(PreventiveObservation.member_id == m.id, PreventiveObservation.status == "active"))

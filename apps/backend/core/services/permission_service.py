...
class PermissionService:
    ...
    async def verify_access(
        self, actor_id: str, target_id: str, resource_type: str, scope: str, resource_id: Optional[str] = None
    ) -> bool:
        """
        ...
        if not actor or not target:
            await self._audit.log_audit_event(
                actor_id, "verify_access", f"{resource_type}:{scope}", "denied - not found", target_id
            )
            return False

        # Fetch Explicit Grants
        grants_res = await self._db.execute(
            select(DataGrant).where(DataGrant.grantee_id == actor.id, DataGrant.grantor_id == target.id)
        )
        grants = grants_res.scalars().all()
        explicit_grants = [{"scope": g.scope, "expires_at": g.expires_at} for g in grants]

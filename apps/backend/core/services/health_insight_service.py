...
class InsightGenerationSchema(BaseModel):
    detected_changes: List[str] = Field(
        default_factory=list, description="New diagnoses, new medications, abnormal lab results."
    )
...
class HealthInsightService:
    ...
    async def generate_insights(
        self, member_id: UUID, family_id: UUID, new_data: str, current_context_summary: str
    ) -> None:
        """
        ...
        try:
            raw_dict = await self._llm.complete_structured(
                self.SYSTEM_PROMPT, prompt, response_schema=InsightGenerationSchema, max_tokens=1500
            )

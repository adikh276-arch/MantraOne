    ...
    async def get_family_graph(self, family_id: UUID) -> dict:
        ...
        for diagnosis in diagnoses.scalars().all():
            ...
            if diagnosis.member_id in member_map:
                edges.append({"source": member_map[diagnosis.member_id], "target": diag_id, "label": "has_diagnosis"})
        ...
        for med in medications.scalars().all():
            ...
            if med.member_id in member_map:
                edges.append({"source": member_map[med.member_id], "target": med_id, "label": "prescribed"})
        ...
        for doc in documents.scalars().all():
            ...
            if doc.member_id in member_map:
                edges.append({"source": member_map[doc.member_id], "target": doc_id, "label": "owns_document"})
        ...
        for metric in metrics.scalars().all():
            ...
            if metric.member_id in member_map:
                edges.append({"source": member_map[metric.member_id], "target": metric_id, "label": "recorded_metric"})
            if metric.source_document_id:
                edges.append(
                    {"source": f"doc_{metric.source_document_id}", "target": metric_id, "label": "extracted_from"}
                )

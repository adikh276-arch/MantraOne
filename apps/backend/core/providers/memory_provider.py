...
class MemoryProvider(IMemoryProvider):
        ...
        cognee.config.set_llm_config(
            {
                ...
                "llm_endpoint": settings.groq_base_url,
            }
        )
    ...
    async def recall(
        self, query: str, family_id: UUID, member_id: UUID, limit: int = 10, min_confidence: float = 0.5
    ) -> list[MemoryFragment]:
        await self._ensure_configured()
        return []

    async def get_health_context(
        self, member_id: UUID, include_domains: list[str], lookback_days: int = 30, family_id: UUID | None = None
    ) -> HealthContext:
        await self._ensure_configured()
    ...
    async def forget(
        self, family_id: UUID, member_id: UUID | None = None, before_date: datetime | None = None
    ) -> ForgetResult:
        await self._ensure_configured()

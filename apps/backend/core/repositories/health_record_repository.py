...
class HealthMetricRepository(BaseRepository[HealthMetric]):
    ...
    async def list_by_type(self, member_id: UUID, family_id: UUID, metric_type: str, limit: int = 50, start_date: datetime | None = None, end_date: datetime | None = None) -> list[HealthMetric]:
        conditions = [
    ...
    async def list_recent(self, member_id: UUID, family_id: UUID, metric_types: list[str], limit: int = 100) -> list[HealthMetric]:

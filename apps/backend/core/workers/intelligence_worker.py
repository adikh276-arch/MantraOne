...
async def process_memory_updated_event(
    ctx: Dict[str, Any], family_id: UUID, member_id: UUID, new_data_text: str, entities: list[dict], memory_type: str
) -> None:
    """
    ...
    async for db in get_async_session():
        ...
        for entity in entities:
            ...
            res = await db.execute(
                select(DomainConfidence).where(
                    DomainConfidence.member_id == member_id, DomainConfidence.domain == domain
                )
            )

...
async def verify():
    ...
    async with httpx.AsyncClient(base_url=BASE_URL, headers=HEADERS) as client:
        ...
        print("\n[*] 3. Create Conversation")
        r = await client.post(
            "/v1/conversations/", json={"family_id": family_id, "member_id": family_id, "title": "Initial Chat"}
        )
        ...
        async with client.stream(
            ...
            json={"family_id": family_id, "member_id": family_id, "text": "Hi, I am John. I sleep 4 hours a night."},
        ) as response:
        ...
        async with client.stream(
            "POST",
            f"/v1/conversations/{conv_id}/messages",
            json={"family_id": family_id, "member_id": family_id, "text": "What do you know about my sleep?"},
        ) as response:
            assert response.status_code == 200, "Message streaming failed"
            first_token = True
            print("  Response: ", end="")

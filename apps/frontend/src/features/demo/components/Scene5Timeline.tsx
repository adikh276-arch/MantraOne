...
const events = [
  {
    ...
    description:
      "Lab report ingested showing a spike in HbA1c. Medication adjusted to Metformin 500mg by Dr. Gupta.",
  ...
  },
  {
    ...
    description:
      "Routine checkup showed stable vitals. No medication prescribed at the time.",
    ...
    <div className="w-full max-w-4xl flex flex-col items-center">
      <div className="w-full text-center mb-16">
        ...
        <p className="text-neutral-400 text-lg">
          Every memory grounds MantraOne in your history.
        </p>
      </div>
...
function TimelineItem({
  ...
  return (
    ...
    >
      <div
        className={`absolute left-[20px] top-1 w-10 h-10 rounded-full flex items-center justify-center border-4 border-neutral-950 transition-colors duration-500 z-10 ${active ? "bg-red-500/20 text-red-500 shadow-[0_0_20px_rgba(239,68,68,0.4)] border-neutral-900" : "bg-neutral-900 text-neutral-500 group-hover:bg-neutral-800"}`}
      >
        {icon}
      </div>

      <div
        className={`rounded-3xl p-8 transition-colors duration-500 border relative overflow-hidden ${active ? "bg-red-950/10 border-red-900/30" : "bg-[#0f0f0f] border-neutral-800 group-hover:border-neutral-700"}`}
      >
        {active && (
          <div className="absolute inset-0 bg-gradient-to-r from-red-500/5 to-transparent pointer-events-none" />
        )}

        <p className="text-sm font-mono text-neutral-500 mb-3 tracking-widest uppercase">
          {date}
        </p>
        <h4
          className={`text-2xl font-semibold mb-4 tracking-tight ${active ? "text-red-400" : "text-white"}`}
        >
          {title}
        </h4>

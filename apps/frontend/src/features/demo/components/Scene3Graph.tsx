...
export function Scene3Graph({
  ...
  const { data: graphData } = useQuery({
    ...
    queryFn: async () => {
      const res = await fetch(
        `http://localhost:8000/v1/memory/graph?family_id=${familyData?.id}`,
      );
      if (!res.ok) throw new Error("Failed");
      ...
      graphData.nodes.forEach((node: any, idx: number) => {
        timers.push(
          setTimeout(
            ...
            Math.min(idx * 300, 2000),
          ),
        ); // Cap animation time for large graphs
      });
  ...
  const getNodeIcon = (type: string) => {
    switch (type) {
      ...
      case "doctor":
        return <Stethoscope className="w-6 h-6 text-purple-400" />;
      case "document":
        return <FileText className="w-6 h-6 text-indigo-400" />;
      default:
        return <User className="w-6 h-6 text-neutral-400" />;
    }
  };
    ...
    <div className="w-full h-[80vh] flex flex-col relative">
      <div className="absolute top-0 left-0 p-8 z-10 pointer-events-none">
        ...
        <p className="text-neutral-400 mt-2 max-w-md">
          MantraOne continuously weaves new medical data into a persistent
          family graph. Relationships are mapped automatically.
        </p>
      </div>
          ...
          {(() => {
            const getMockPos = (id: string) => {
              ...
              if (isPres) {
                ...
                const nonCenterNodes = nodes.filter(
                  (n) => n.id !== nodes[0]?.id,
                );
              ...
              <>
                ...
                {nodes.map((node, i) => {
                  ...
                  return (
                    ...
                    >
                      <div
                        className={`w-16 h-16 rounded-full flex items-center justify-center border-4 ${node.id === nodes[0]?.id ? "bg-blue-900 border-blue-500 shadow-[0_0_30px_rgba(59,130,246,0.3)]" : "bg-neutral-900 border-neutral-800"}`}
                      >
                        {getNodeIcon(node.type)}
                      </div>
                      <div className="bg-black/80 backdrop-blur border border-neutral-800 px-4 py-2 rounded-full whitespace-nowrap">
                        <p
                          className={`text-sm font-medium ${node.id === nodes[0]?.id ? "text-white" : "text-neutral-300"}`}
                        >
                          {node.label}
                        </p>
                        <p className="text-[10px] text-neutral-500 uppercase tracking-widest text-center mt-0.5">
                          {node.type}
                        </p>
                      </div>
                    </motion.div>

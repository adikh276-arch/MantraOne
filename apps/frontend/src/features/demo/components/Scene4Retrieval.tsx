...
export function Scene4Retrieval({ onNext, mode }: { onNext: () => void, mode: "live" | "presentation" }) {
  ...
  const PRESENTATION_ANSWER = "Based on the recent Lab Report, Anil's HbA1c has escalated to 8.2%. Metformin 500mg was prescribed.";
  ...
  const triggerSearch = async (e: React.FormEvent) => {
    ...
    if (mode === "presentation") {
      ...
      setTimeout(() => {
        ...
        const interval = setInterval(() => {
          ...
          if (i > PRESENTATION_ANSWER.length) {
            ...
            setTimeout(() => {
              setChatHistory(prev => [...prev, {role: 'assistant', content: PRESENTATION_ANSWER}]);
              setCurrentAnswer("");
              setPhase("idle");
              setRetrievedDocs([]);
            }, 1000);
      ...
      try {
        ...
        setTimeout(() => setRetrievedDocs(prev => [...prev, "Traversing Memory Graph..."]), 500);
        setTimeout(() => setRetrievedDocs(prev => [...prev, "Extracting context..."]), 1000);
        
        setTimeout(async () => {
           setPhase("answering");
           try {
             const streamRes = await fetch(`http://localhost:8000/v1/conversations/${activeConvId}/messages`, {
               method: 'POST',
               headers: { 'Content-Type': 'application/json' },
               body: JSON.stringify({ family_id: familyData.id, member_id: familyData.primary_user_id || familyData.id, text: currentQuery })
             });
             ...
             setTimeout(() => {
               setChatHistory(prev => [...prev, {role: 'assistant', content: streamedText}]);
               setCurrentAnswer("");
               setPhase("idle");
               setRetrievedDocs([]);
             }, 1000);
         ...
         setTimeout(() => {
           setChatHistory(prev => [...prev, {role: 'assistant', content: "Failed to connect to backend."}]);
           setCurrentAnswer("");
           setPhase("idle");
           setRetrievedDocs([]);
         }, 1000);
  ...
  return (
    <div className="w-full max-w-4xl flex flex-col min-h-[70vh] pb-32">
                ...
                <div className="w-full bg-neutral-950 border border-neutral-800 rounded-3xl p-8 relative overflow-hidden shadow-2xl">
                   ...
                   <div className="flex justify-between items-start mb-6 pb-4 border-b border-neutral-800/50">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                           <h4 className="font-semibold text-white tracking-tight">MantraOne</h4>
                        </div>
                      </div>
                   </div>
          ...
          {(phase === "searching" || phase === "answering") && (
            ...
            >
              <div className="w-full bg-neutral-950 border border-neutral-800 rounded-3xl p-8 relative overflow-hidden shadow-2xl">
                 ...
                 {phase === "searching" ? (
                   <div className="flex items-center gap-4">
                      <motion.div animate={{ rotate: 360 }} transition={{ repeat: Infinity, duration: 4, ease: "linear" }} className="w-10 h-10 rounded-xl border border-blue-500/50 flex items-center justify-center bg-blue-500/10">
                         <Search className="w-5 h-5 text-blue-400" />
                      </motion.div>
                      <div>
                         <h3 className="text-lg font-medium text-white tracking-tight mb-2">Traversing Memory Graph...</h3>
                         {retrievedDocs.map((doc, idx) => (
                           <div key={idx} className="flex items-center gap-2 text-sm text-neutral-500 mt-1">
                             <CheckCircle className="w-3 h-3 text-green-500" /> {doc}
                           </div>
                         ))}
                      </div>
                   </div>
      ...
      <div className="w-full max-w-4xl fixed bottom-8 px-4 z-20">
         <form onSubmit={triggerSearch} className="relative w-full group shadow-2xl">
           <input 
             disabled={phase !== "idle"}
             type="text" 
             value={query}
         ...
         {chatHistory.length > 0 && (
           <div className="flex justify-center mt-6">
              <button onClick={onNext} className="bg-white text-black px-6 py-3 rounded-full font-medium hover:scale-105 transition-transform shadow-2xl">
                See Timeline Update
              </button>
           </div>
         )}

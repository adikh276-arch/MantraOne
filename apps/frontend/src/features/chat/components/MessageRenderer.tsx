...
export function MessageRenderer({ content }: MessageRendererProps) {
      ...
      return (
        <div className="space-y-2">
          ...
          <div className="bg-orange-50 border border-orange-100 p-4 rounded-xl shadow-sm mt-2">
            ...
            {data.reason && (
              <p className="text-xs text-orange-600 mt-3 border-t border-orange-100/50 pt-2">
                {data.reason}
              </p>
            )}
          </div>
  ...
  return (
    <p className="text-stone-800 leading-relaxed text-sm md:text-base whitespace-pre-wrap">
      {content}
    </p>
  );
}

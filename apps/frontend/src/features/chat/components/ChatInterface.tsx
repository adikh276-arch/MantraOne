      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-6">
        <AnimatePresence>
          {messages.length === 0 && (
            ...
            >
              <p className="text-lg font-medium text-stone-600">How is your family today?</p>
              <p className="text-sm text-center max-w-xs">Upload a report, record a voice memo, or just say hello to get started.</p>
            </motion.div>
          )}
          ...
          {messages.map((msg) => (
            ...
            >
              ...
              >
                ...
                {msg.status === 'error' && (
                  <span className="block mt-1 text-xs text-red-400">Failed to send. Click to retry.</span>
                )}
              </div>
            </motion.div>
          ))}
